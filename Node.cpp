...
Node* Node::findPredecessor(int key, vector<Node*>& path){
    ...
    while(keyInNodeRangeInclusive(key, possiblePredecessor->getHashId(), possiblePredecessor->getSuccessor()->getHashId() )==false){
...
Node* Node::closestPrecedingFinger(int key){
    for(int i= M; i>=1; i--){
        if( (getFingerTable().getNextNode(i)->getHashId() != this->getHashId()) && keyInNodeRangeInclusive(getFingerTable().getNextNode(i)->getHashId(), getHashId(), key)){
...
void Node::updateFingerTableJoinOP(Node* node, int index){
    ...
    if(node != this){
        if( keyInNodeRangeStartInclusive(node->getHashId(), this->hash_id, successor_node->hash_id)){
...
void Node::migrateKeysOnLeave() {
    ...
    auto keys = getLocalKeys();

    if (successor != this) {
        std::cout << "===== Initiating Key Migration =====\n";
        for (const auto& keyValuePair : keys) {
            int key = keyValuePair.first;
            int value = keyValuePair.second;

            std::cout << "Transferring key " << key
                      << " from Node " << getId()
                      << " to Node " << successor->getId() << ".\n";
...
bool keyInNodeRangeInclusive(int key, int startNode, int endNode)
{
    if(startNode < endNode){
        return key<=endNode && key>=startNode;
    }
    else{
        return key<=endNode || key>=startNode;
    }
